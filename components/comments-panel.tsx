        ...
        <AccordionItem value={thread.id} className="border-0">
          <AccordionPrimitive.Header className="flex">
            <AccordionPrimitive.Trigger
              disabled={resolved}
              className={cn(
                "group/thread-trigger w-full px-4 py-3 text-left outline-none",
                !resolved && "hover:bg-accent/30 cursor-pointer transition-colors"
              )}
            >
                  ...
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    {!resolved && (
                      <span className="ml-auto shrink-0">
                        <ChevronDown className="size-3 text-muted-foreground transition-transform duration-200 group-aria-expanded/thread-trigger:rotate-180" />
                      </span>
                    )}
                  </div>
